...
export const metricsInputSchema = zod.object({
  ...
  temperature: zod
    .number()
    .min(-20, "Temp cant be below -20 celsius")
    .max(60, "Temp can't exceed 60 celsius"),
  wind_speed: zod
    .number()
    .min(0, "Wind speed can't be below  0 m/s")
    .max(200, "Wind speed can't exceed 200 m/s"),
  humidity: zod
    .number()
    .min(0, "Humidity level can't be below 0%")
    .max(100, "Humidity level can't exceed 100%"),
  pressure: zod
    .number()
