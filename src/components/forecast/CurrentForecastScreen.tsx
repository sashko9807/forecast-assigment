...
export default function CurrentWeatherScreen() {
  ...
  return(
    <div className='flex flex-col sm:items-start sm:flex-row'>
      ...
      <CurrentForecastCard currentWeather={dailyWeatherData.current} offset={dailyWeatherData.offset}/>
      </div>
      ...
      <SevenDaysForecastCard dailyForecast={dailyWeatherData.daily} offset={dailyWeatherData.offset}/>
      </div>
    </div>
  )
}
  ...
  return (
    ...
    <div className='sm:w-[330px]  bg-white'>
      ...
      <div className='flex flex-row items-center  justify-between '>
        ...
        <Image priority src={openWeatherImageUrl (currentWeather.weather[0].icon)} fill alt='weather-icon' />
        </div>
      <div className='flex flex-row justify-end'>
      <span className='text-orange-500 text-5xl break-normal'>{Math.floor(currentWeather.temp)} </span>
      <span className='text-orange-500 text-2xl whitespace-normal'>°C</span>
      </div>
      </div>
      </div>
      <div className='flex justify-center'>
        <h1 className='text-xl font-medium   text-center w-2/4'>{currentWeather.weather[0].description} </h1>
      </div>
      <div className='flex flex-row justify-center'>
        <h2 className='text-lg'>{"Усеща се като"}</h2> &nbsp; 
        <div className='flex flex-row'>
        <span className='text-lg'>{Math.floor(currentWeather.feels_like)}</span>
      ...
      <div className='grid grid-cols-6 gap-1 mt-5 sm:mt-8 sm:relative'>
        <div className='flex flex-col items-center'>
          ...
          <h1 className='text-sm text-[#92D3ED] font-bold text-center'>{currentWeather.humidity}%</h1>
        </div>
         <div className='flex flex-col items-center'>
          <Image priority src={'/icons/wind.svg'} alt='wind' width={25} height={30}/>
          <h1 className='text-sm text-[#92D3ED] text-center font-bold'>{Math.ceil(currentWeather.wind_speed)} м/с</h1>
        </div>
          ...
          <h1 className='text-[13px]  sm:text-sm text-center text-[#c3c4c5]'>Изгрев {getHoursAndDate(currentWeather.sunrise, 0).hour}</h1>
        </div>
        <div className='flex flex-col col-start-5 col-end-7 items-center'>
          ...
          <h1 className='text-[13px] sm:text-sm text-center text-[#c3c4c5] '>Залез {getHoursAndDate(currentWeather.sunset, 0).hour}</h1>
        </div>
      </div>
    </div>
  )
...
<>
 {
  dailyForecast.map((weatherData:any, idx:any) => {
    ...
    return (
      <div key={weatherData.dt} className='relative sm:border-r-2 border-t-2 sm:border-t-0 last:border-r-0 border-dashed sm:bg-[#34313B] bg-white'>
      <div  className=' flex flex-wrap sm:min-w-[290px] sm:flex-col sm:bg-[#34313B] sm:border-t-0 items-center py-4 px-4 w-full bg-white justify-between'>
        ...
        <div className='flex  flex-row items-center gap-4 justify-end max-w-full sm:flex-col'>
          ...
          <Image priority src={openWeatherImageUrl(weatherData.weather[0].icon)} fill alt='weather-icon' style={{aspectRatio:1}}/>
          </div>
          ...
          <span className='hidden sm:flex justify-center text-lg text-white font-bold '>{weatherData.weather[0].description}</span>         
